...
const TABS = ["Balances", "Positions", "Open Orders", "TWAP", "Trade History", "Funding History", "Order History"] as const
...
const HyperLiquid: React.FC = () => {
    ...
    const transformBalances = (balances: SpotClearinghouseState): TableRow[] => {
        ...
        return balances.balances.map((balance) => ({
            ...
            availableBalance:
                balance.total && balance.hold ? (parseFloat(balance.total) - parseFloat(balance.hold)).toFixed(4) : "0.0000",
            usdcValue: balance.coin === "USDC" ? `$${parseFloat(balance.total).toFixed(2)}` : "$0.00",
            pnl: "+0.00 (0.00%)",
    ...
    const transformPositions = (positions: ClearinghouseState): TableRow[] => {
        ...
        return positions.assetPositions.map((position) => ({
            ...
            pnl: position.position?.unrealizedPnl
                ? `${parseFloat(position.position.unrealizedPnl) >= 0 ? "+" : ""}${parseFloat(position.position.unrealizedPnl).toFixed(2)}`
                : "+0.00",
    ...
    const transformOpenOrders = (orders: FrontendOpenOrders): TableRow[] => {
        ...
        return orders.map((order) => ({
            ...
            orderValue: order.sz && order.limitPx ? `$${(parseFloat(order.sz) * parseFloat(order.limitPx)).toFixed(2)}` : "$0.00",
    ...
    const transformTradeHistory = (trades: UserFills): TableRow[] => {
        ...
        return trades.map((trade) => ({
            ...
            tradeValue: trade.sz && trade.px ? `$${(parseFloat(trade.sz) * parseFloat(trade.px)).toFixed(2)}` : "$0.00",
            fee: trade.fee ? parseFloat(trade.fee).toFixed(2) : "0.00",
            closedPnl: trade.closedPnl
                ? `${parseFloat(trade.closedPnl) >= 0 ? "+" : ""}${parseFloat(trade.closedPnl).toFixed(2)}`
                : "+0.00",
        }))
    }
    ...
    const transformFundingHistory = (funding: UserFunding): TableRow[] => {
        ...
        return funding.map((entry) => ({
            ...
            positionSide: entry.delta?.szi ? (parseFloat(entry.delta.szi) > 0 ? "Long" : "Short") : "-",
            payment: entry.delta?.usdc ? `$${parseFloat(entry.delta.usdc).toFixed(4)}` : "$0.0000",
            rate: entry.delta?.fundingRate ? `${(parseFloat(entry.delta.fundingRate) * 100).toFixed(4)}%` : "0.0000%",
        }))
    }
    ...
    const transformOrderHistory = (orders: HistoricalOrder[]): TableRow[] => {
        ...
        return orders.map((order) => ({
            ...
            filledSize: order.status === "filled" && order.order?.sz ? parseFloat(order.order.sz).toFixed(4) : "0.0000",
            orderValue:
                order.order?.sz && order.order?.limitPx
                    ? `$${(parseFloat(order.order.sz) * parseFloat(order.order.limitPx)).toFixed(2)}`
                    : "$0.00",
            price: order.order?.limitPx ? `$${parseFloat(order.order.limitPx).toFixed(2)}` : "Market",
            reduceOnly: order.order?.reduceOnly ? "Yes" : "No",
            triggerConditions: order.order?.triggerPx ? `$${parseFloat(order.order.triggerPx).toFixed(2)}` : "-",
    ...
    return (
        <div className="min-h-screen flex bg-[#101616] text-[#E6FFF6]">
            ...
            <main className="flex-1 p-4 md:p-8 overflow-y-auto">
                ...
                {error && <div className="mb-4 p-4 bg-red-900/50 text-red-300 rounded-lg">{error}</div>}
                ...
                >
                    {TABS.map((tab) => (
                        <motion.button
                            key={tab}
                            className={`px-4 py-2 text-base font-medium border-b-2 transition-colors whitespace-nowrap cursor-pointer ${
                                activeTab === tab
                        ...
                        {isLoading ? (
                            ...
                            <table className="min-w-full text-left">
                                ...
                                <tbody>
                                    {activeData.length === 0 ? (
                                        <tr>
                                            <td colSpan={activeColumns.length} className="py-4 px-4 text-[#E6FFF6] text-center">
                                                {noDataMsg}
                                            </td>
                                        </tr>
                                    ) : (

...
const LoopCycleExecutor = () => {
    ...
    const { swapUSDCToUSDT, transferUSDCToPerp, openHypePosition } = useCoreActions()
    ...
    const executeCoreFlow = async () => {
        try {
            ...
            parsedSize = parseUnits(convertedSize.toString(), decimals)

            await openHypePosition(false, parsedPrice, parsedSize)
            ...
            alert(`Core flow failed: ${error instanceof Error ? error.message : String(error)}`)
        } finally {
            setIsLoading(false)
    ...
    const executeFullLoopCycle = async () => {
        ...
        try {
            ...
            alert(`Full loop cycle failed: ${error instanceof Error ? error.message : String(error)}`)
        } finally {
            setIsLoading(false)
            setIsModalOpen(false)
        }
                    ...
                    >
                        ...
                        >
                            <div className="text-center">
                                <div className="mb-6">
                                    <h3 className="text-xl font-semibold text-[#E6FFF6] mb-2">Executing Full Loop Cycle</h3>
                                    ...
                                    {executionSteps.map((step, index) => (
                                        <motion.div
                                            ...
                                            className={`flex items-center space-x-3 p-3 rounded-lg ${
                                                index <= currentStep ? "bg-[#1A2323] border border-[#00FFB2]/20" : "bg-[#0B1212]"
                                            }`}
                                        >
                                            <div
                                            ...
                                            <span
                                                className={`text-sm ${
                                                    index <= currentStep ? "text-[#E6FFF6]" : "text-[#A3B8B0]"
                                                }`}
                                            >
                                                {step}
                                            </span>
    ...
    return (
        <div className="bg-[#0B1212] rounded-xl p-6">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h2 className="text-xl font-semibold text-[#E6FFF6]">Loop Cycle Executor</h2>
                    <p className="text-[#A3B8B0] text-sm mt-1">
                        Execute a full leveraged loop cycle with flashloan and position management
                    </p>
                </div>
            ...
            <div className="bg-[#0B1212] rounded-xl p-6 mb-6">
                <div className="mb-4">
                    <h3 className="text-lg font-semibold text-[#E6FFF6]">Full Strategy Flow</h3>
                    <p className="text-[#A3B8B0] text-sm">Automated execution of the complete leveraged loop cycle</p>
                </div>

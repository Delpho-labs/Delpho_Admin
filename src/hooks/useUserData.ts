...
import { DELPHO_CONFIG_PROVIDER_ABI, DELPHO_STABILITY_POOL_ABI, DELPHO_STAKER_ABI, DELPHO_VAULT_LENS_ABI } from '../config/Abi';
import { hyperliquidMainnet } from '../config/chains';
import { DELPHO_CONFIG_PROVIDER_ADDRESS, DELPHO_STABILITY_POOL_ADDRESS, DELPHO_STAKER_ADDRESS, DELPHO_VAULT_ADDRESS, DELPHO_VAULT_LENS_ADDRESS } from '../config/constants';
...
export function useUserData(userAddress : Address): UserMetrics {
    ...
    const { data: stabilityPoolData, isLoading, error } = useQuery({
        ...
        queryFn: async (): Promise<UserMetrics> => {
            ...
            try {
                ...
                rawDepositedAssets.forEach((asset, index) => {
                    ...
                    const displayName = assetInfo.symbol !== 'UNKNOWN' 
                        ? assetInfo.symbol 
                ...
                allSupportedTokens.forEach((token) => {
                    ...
                    const isAlreadyAdded = rawDepositedAssets.some(asset => 
                        asset.toLowerCase() === tokenLower
                    );

...
import { EXECUTOR_ADDRESS, K_HYPE, USDT, W_HYPE } from "../config/constants"
import {
...
import { truncate, calculatePriceWithSlippage, wait, calculateBorrowAmountRaw } from "../utils/helper.ts"
...
export function useCoreActions() {
    ...
    const handleUsdtToUsdcSwap = useCallback(
        async (swapAmount: number, executorAddress: string = EXECUTOR_ADDRESS): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const transferUSDCToPerp = useCallback(
        async (amount: number, executorAddress: string = EXECUTOR_ADDRESS): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const openHypeShort = useCallback(
        async (amount: number, leverage: number = 4, executorAddress: string = EXECUTOR_ADDRESS): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const closeHypeShort = useCallback(
        async (size: number, executorAddress: string = EXECUTOR_ADDRESS): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    // Hyperlend Functions
    const executeFullEvmFlowHyperlend = useCallback(
        async (collateralAsset: string, executorAddress: string): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const closeLeveragePositionHyperlend = useCallback(
        async (asset: string, executorAddress: string): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const partialClosePositionHyperlend = useCallback(
        async (asset: string, khypeWithdrawAmount: bigint, executorAddress: string): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const calculatePartialCloseAmountsHyperlend = useCallback(
        async (
            asset: string,
            ...
            try {
                ...
                const collateralAmount = await publicClient
                    .readContract({
    ...
    // Sentiment Functions
    const executeFullEvmFlowSentiment = useCallback(
        async (vaultAddress: string, positionAddress: string, executorAddress: string): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const closeLeveragePositionSentiment = useCallback(
        async (
            asset: string,
            ...
            try {
                ...
                while (Number(quoteResult.effectiveInputAmount) < Number(debtAmount) && attempts < maxAttempts) {
                    ...
                    attempts++
                    console.log(
                        `Attempt ${attempts}: effectiveInputAmount (${quoteResult.effectiveInputAmount}) < debtAmount (${debtAmount})`
                    )
    ...
    const calculatePartialCloseAmountsSentiment = useCallback(
        async (
            asset: string,
            executorAddress: string,
            positionAddress: string,
    ...
    const repayUSDTSentiment = useCallback(
        async (amount: bigint = 0n, executorAddress: string): Promise<boolean> => {
            if (!walletClient || !address) throw new Error("Wallet not connected")
    ...
    const partialClosePositionSentiment = useCallback(
        async (
            positionAddress: string,
            debtAmount: bigint,
            khypeWithdrawAmount: bigint,
